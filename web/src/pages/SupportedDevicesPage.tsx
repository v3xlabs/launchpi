...
const DeviceTile: Component<{ device: SupportedDevice }> = (props) => (
    <div class="card flex items-center gap-3 p-3">
        <DeviceImage model={props.device.model} class="h-12 w-20" />
        <div class="min-w-0 flex-1">
            <p class="row-title">{props.device.model}</p>
            <p class="row-meta">
                {props.device.grid}
                <Show when={props.device.extras}>{(extras) => <> · {extras()}</>}</Show>
            </p>
        </div>
        <div class="flex shrink-0 flex-col items-end gap-1">
            <span class="chip chip-muted">{connectionLabels[props.device.connection]}</span>
            <Show when={!props.device.isImplemented}>
