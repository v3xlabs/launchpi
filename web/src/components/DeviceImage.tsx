...
type DeviceImageProps = { model: string; class?: string };

export const DeviceImage: Component<DeviceImageProps> = (props) => (
    <Show when={deviceImageSlug(props.model)}>
        {(slug) => (
            <img
                ...
                alt=""
                classList={{ 'device-image': true, [props.class ?? '']: true }}
                loading="lazy"
            />
        )}
    </Show>
